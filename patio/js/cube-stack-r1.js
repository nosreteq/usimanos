...
var CubeStack = (function () {
  ...
  var createPlane = function (opt) {
    opt = opt || {};
    var planeColor =
      opt.colors[opt.planeColor === undefined ? 1 : opt.planeColor];
    var plane = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({
        ...
        map: datatex.seededRandom.apply(
          null,
          [opt.gw * 4, opt.gh * 4].concat(planeColor)
        ),
        emissive: 0x0a0a0a,
  ...
  var appendBoxMeshObjects = function (stack, opt) {
    ...
    while (boxIndex < opt.boxCount) {
      ...
      if (cubeStack) {
        ...
        var cubeColor =
          opt.colors[Math.floor(opt.colors.length * Math.random())];
